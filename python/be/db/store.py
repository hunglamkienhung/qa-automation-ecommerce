"""Read-only access to the mini-shop SQLite file. Mirror of node/be/db/store.js.
...
to memory-map it -- set MINI_SHOP_DB to a /tmp path when running under WSL.
"""
...
DB_FILE = Path(os.environ.get("MINI_SHOP_DB") or Path(__file__).resolve().parents[3] / "services" / "mini-shop" / "data" / "mini-shop.db")
SCHEMA = Path(__file__).resolve().parents[3] / "services" / "mini-shop" / "db" / "schema.sql"
SEED = Path(__file__).resolve().parents[3] / "services" / "mini-shop" / "db" / "seed.sql"
...
class Store:
    def __init__(self, file: Path = DB_FILE) -> None:
        ...
        if not self.file.exists():
            raise DbUnreachable(f"no store at {self.file} -- start mini-shop (services/mini-shop/serve.sh up)")
        try:
            self.db = sqlite3.connect(f"file:{self.file.as_posix()}?mode=ro", uri=True, timeout=5, isolation_level=None)
            self.db.execute("SELECT 1 FROM products").fetchall()
    ...
    def ledger_stock(self, product_id):
        return int(self.get("SELECT COALESCE(SUM(delta),0) AS s FROM stock_movements WHERE product_id = ?", product_id)["s"])
