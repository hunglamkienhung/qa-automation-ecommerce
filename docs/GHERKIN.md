...
- `@module:NN-name`, and a tier tag: `@be`/`@fe` with `@db`/`@api`, plus a
  target tag (`@minishop`, `@site`). The two stacks select the same set —
...
```gherkin
But "the live site's total order count" cannot be verified because
    "the public API exposes no orders endpoint"
```
